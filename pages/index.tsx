...
export default function Home({ vehicleData }: { vehicleData: OffersData }) {
  ...
  let areFiltersUpdated = false, aure
  ...
  const {
    ...
    gearbox,
    exteriorColor,
    category }: QueryParams = router.query
    ...
    const urlFilters = useMemo(() => ({
      ...
      ...(!!category && { category })
    }), [appliedFilters, category, exteriorColor, firstRegistration, fuel, gearbox, make, mileage, model, power, price])

  
  useEffect(() => {
    const areUrlFiltersApplied = Object.values(urlFilters)
    .some((filterVal) => !!filterVal)
